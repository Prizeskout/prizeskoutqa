export default async ({project})=>{
  ...
  const enter=(dx=0,dy=70,rot=0,delay=.04)=>({enter:{from:{x:dx,y:dy,scale:.88,opacity:0,rotation:rot},duration:.38,at:delay,easing:"house"},settle:{to:{x:-dx*.14,y:-dy*.12,scale:1.038,rotation:-rot*.12},duration:2.16,easing:"ease-out"},exit:{to:{x:-dx*.22,y:-dy*.18,scale:1.075,opacity:0},duration:.2,anchor:"end"}});
  const fly=(dx,dy,rot,delay=.02)=>({enter:{from:{x:dx,y:dy,scale:.48,opacity:0,rotation:rot},duration:.38,at:delay,easing:"house"},settle:{to:{x:-dx*.12,y:-dy*.11,scale:1.08,rotation:-rot*.12},duration:1.12,easing:"ease-out"},exit:{to:{x:-dx*.34,y:-dy*.28,scale:1.3,opacity:0},duration:.2,anchor:"end"}});
  ...
  const stream=(y,color,delay)=> <><rect x={150} y={y} width={1620} height={2} radius={1} fill={color} opacity={.1}/><rect x={150} y={y} width={1620} height={3} radius={2} fill={color} opacity={.72} animate={[{property:"scaleX",from:0,to:1,at:delay,duration:.62,easing:"ease-out"}]}/><frame layout="none" x={1600} y={y-7} width={16} height={16} radius={8} background={color} motion={{enter:{from:{x:-1420,scale:.2,opacity:0},duration:.7,at:delay,easing:"house"},exit:{to:{scale:1.8,opacity:0},duration:.2,anchor:"end"}}}/></>;
