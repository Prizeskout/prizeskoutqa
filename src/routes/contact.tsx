...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      { title: "Talk to Our Team | PrizeSkout" },
      {
        name: "description",
        content:
          "Talk to PrizeSkout about margin intelligence, integrations, partnerships, or your commerce operation.",
      },
      { property: "og:title", content: "Talk to Our Team | PrizeSkout" },
      {
...
function ContactPage() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      companyType: String(fd.get("company_type") ?? "").trim(),
  ...
  return (
    <main className="demo-contact">
      ...
      <header className="demo-contact-hero">
        <div><span>TALK TO OUR TEAM</span><h1>Let’s start a<br/><em>conversation.</em></h1><p>Tell us a little about your business and what you would like to improve. We will connect you with the right person.</p><div className="demo-promises"><span>✓ A reply within one business day</span><span>✓ The right product, integration, or partnership contact</span><span>✓ Clear next steps with no pressure</span></div></div>
        <aside><small>WHAT TO EXPECT</small><strong>A focused conversation with PrizeSkout</strong><p>We will start with your goals and only bring in the details that matter.</p><div><span>01</span>We review your message</div><div><span>02</span>Connect you with the right person</div><div><span>03</span>Agree the next step</div></aside>
      </header>
      <section className="demo-contact-body">
        ...
        >
          ...
          <p style={{ fontSize: 13, color: "#6B6B6B", marginTop: 6 }}>
            Share the essentials. We will reply within one business day.
          </p>
          ...
          {submitted ? (
            ...
            <form onSubmit={onSubmit} style={{ marginTop: 20, display: "grid", gap: 14 }}>
              <div style={{ display: "grid", gap: 14, gridTemplateColumns: "1fr 1fr" }}>
                ...
                <Field label="Company" name="company" placeholder="Your company" required maxLength={200} />
              </div>
              <Field
                label="Work email"
                name="email"
              ...
              />
              <div>
                <label htmlFor="company_type" style={labelStyle}>Company type</label>
                <select id="company_type" name="company_type" required style={inputStyle} defaultValue="">
              ...
              <div>
                <label htmlFor="message" style={labelStyle}>What would you like to improve? <span style={{ color: "#7B8597", fontWeight: 600 }}>Optional</span></label>
                <textarea
                  id="message"
                  name="message"
                  rows={5}
                  maxLength={5000}
                  placeholder="Tell us what you’re exploring or trying to solve."
...
const contactCss = `
  ...
  .demo-contact-body select{min-height:48px}.demo-contact-body select:focus{border-color:#f36a21!important;box-shadow:0 0 0 4px #f36a2112;outline:none}
`;
