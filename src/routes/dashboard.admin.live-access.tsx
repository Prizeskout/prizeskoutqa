...
export const Route = createFileRoute("/dashboard/admin/live-access")({
...
function LiveAccessPage() {
  ...
  const handleRevoke = async (uid: string) => {
    if (!confirm("Revoke live access? All existing live API keys will be revoked immediately."))
      return;
    setBusy(uid);
...
function AccountRow({
  row, busy, onApprove, onReject, onRevoke,
}: {
  row: AccountRow;
  busy: boolean;
  onApprove: () => void;
  ...
  return (
    <div style={{ padding: 16, borderBottom: "1px solid #F1EDE4" }}>
      ...
      {row.decision_note && (
        <div style={{ fontSize: 12, color: "#6B6B6B", marginBottom: 10, fontStyle: "italic" }}>
          Note: {row.decision_note}
        </div>
      )}
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {row.live_status !== "approved" && (
          <button type="button" onClick={onApprove} disabled={busy} style={btn("#16A34A", "#fff")}>
            <Check size={14} /> Approve
          </button>
        )}
        {row.live_status === "pending" && (
          <button type="button" onClick={onReject} disabled={busy} style={btn("#fff", "#991B1B", "#FECACA")}>
            <X size={14} /> Reject
          </button>
        )}
        {row.live_status === "approved" && (
          <button type="button" onClick={onRevoke} disabled={busy} style={btn("#fff", "#991B1B", "#FECACA")}>
