  ...
  const filtered = useMemo(() => {
    ...
    return events.filter((e) => {
      ...
      const sok = !q ||
        ...
        (e.summary_en  ?? "").toLowerCase().includes(q) ||
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
        <button
          type="button"
          onClick={() => setType("all")}
          style={filterBtn(typeFilter === "all")}
        >
          All ({counts.all ?? 0})
        </button>
        {ALL_TYPES.map((t) => (
          <button
            ...
            type="button"
            onClick={() => setType(t)}
            style={filterBtn(typeFilter === t, t)}
          >
            {t} ({counts[t] ?? 0})
          </button>
        ))}
        ...
        <div style={searchWrap}>
          <input
            type="search"
            placeholder="Search merchant, SKU, summary…"
            value={search}
      ...
      <div style={{ overflowX: "auto", border: "1px solid #E5E2DB", borderRadius: 12 }}>
        <table style={{ width: "100%", borderCollapse: "collapse", backgroundColor: "#fff" }}>
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={8} style={{ padding: 24, color: "#8A8A8A", fontSize: 13 }}>
                  Loading audit log…
                </td>
              </tr>
            ) : filtered.length === 0 ? (
              <tr>
                <td colSpan={8} style={{ padding: 32, textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 13, color: "#6B6B6B" }}>
                    {events.length === 0
                      ? "No audit events yet. Events are written when the price pipeline processes its first request."
              ...
              filtered.map((ev, i) => {
                ...
                return (
                  <tr
                    key={ev.trace_id || i}
                    style={{ borderTop: i === 0 ? "none" : "1px solid #F1EFE9" }}
                  >
                    <td
                      style={{
                        padding: "10px 14px",
                    ...
                    >
                      {ev.source_platform || ev.target_channel || "—"}
                    </td>
                    <td
                      style={{
                        padding: "10px 14px",
      ...
      <div style={{ fontSize: 12, color: "#8A8A8A" }}>
        Showing {filtered.length} of {events.length} events (latest 200 loaded). Events are immutable
        once written.
      </div>
    </div>
  );
...
const filterBtn = (active: boolean, type?: string): React.CSSProperties => {
  ...
  return {
    ...
    border: `1px solid ${active ? accent : "#E5E2DB"}`,
    backgroundColor: active ? (type === "error" ? "#FEF2F2" : "#FFF7ED") : "#fff",
    color: active ? accent : "#6B6B6B",
    cursor: "pointer",
    fontFamily: type ? "ui-monospace, SFMono-Regular, monospace" : undefined,
  };
};
