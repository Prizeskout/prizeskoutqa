...
export const Route=createFileRoute("/api/copilot/store")({server:{handlers:{POST:async({request})=>{
  ...
  const nameOf=(value:unknown)=>typeof value==="object"&&value?String((value as Record<string,unknown>).en??(value as Record<string,unknown>).ar??""):String(value??"");
  const findExactProduct=async(queryValue:string)=>{const query=queryValue.trim().toLowerCase();const response=await fetch("https://api.zid.sa/v1/products/?page=1&page_size=100",{headers}),payload=await response.json().catch(()=>({})) as Record<string,unknown>;if(!response.ok)throw new Error(`Zid products returned ${response.status}.`);const rows=(Array.isArray(payload.results)?payload.results:Array.isArray(payload.products)?payload.products:[]) as Array<Record<string,unknown>>,matches=rows.filter(p=>nameOf(p.name).toLowerCase()===query||String(p.sku??"").toLowerCase()===query);if(matches.length!==1)throw new Error(matches.length?`That matched ${matches.length} products. Use an exact product name or SKU.`:"No Zid product matched that exact name or SKU.");return matches[0];};
  if(body?.action==="product_image_upload"){
    try{const product=await findExactProduct(body.query??""),url=new URL(body.image_url??"");if(url.protocol!=="https:")return json({error:"Use a public HTTPS image URL."},400);if(/^(localhost|127\.|10\.|192\.168\.|169\.254\.|172\.(1[6-9]|2\d|3[01])\.)/i.test(url.hostname))return json({error:"Private or local image URLs are not allowed."},400);const source=await fetch(url,{redirect:"follow"});const type=source.headers.get("content-type")??"";if(!source.ok||!type.startsWith("image/"))return json({error:"The supplied URL did not return an image."},422);const bytes=await source.arrayBuffer();if(bytes.byteLength>10*1024*1024)return json({error:"The image is larger than 10 MB."},422);const form=new FormData();form.set("image",new Blob([bytes],{type}),`product.${type.split("/")[1]?.split(";")[0]||"jpg"}`);form.set("alt_text",body.image_alt?.trim()||nameOf(product.name));const id=String(product.id??""),upload=await fetch(`https://api.zid.sa/v1/products/${encodeURIComponent(id)}/images/`,{method:"POST",headers,body:form}),result=await upload.json().catch(()=>({})) as Record<string,unknown>;if(!upload.ok)return json({error:`Zid rejected the image (${upload.status}): ${JSON.stringify(result).slice(0,220)}`},502);const verify=await fetch(`https://api.zid.sa/v1/products/${encodeURIComponent(id)}/images/`,{headers}),list=await verify.json().catch(()=>({})) as Record<string,unknown>,rows=(Array.isArray(list.results)?list.results:Array.isArray(list.images)?list.images:[]) as Array<Record<string,unknown>>,createdId=String(result.id??""),confirmed=Boolean(createdId&&rows.some(row=>String(row.id??"")===createdId));return json({ok:true,confirmed,action_id:`PS-IMAGE-${Date.now().toString(36).toUpperCase()}`,message:confirmed?`The new image is now attached to ${nameOf(product.name)} and confirmed in Zid.`:"Zid accepted the image, but it was not yet visible in readback.",product:{id,name:nameOf(product.name),sku:product.sku},image:result});}catch(error){return json({error:error instanceof Error?error.message:String(error)},422);}
  }
  ...
  if(body?.action==="coupon_change"){
    ...
    if(mode==="create"){
      ...
      const now=new Date(),end=new Date(now);end.setFullYear(end.getFullYear()+1);const form=new FormData();for(const [key,value] of Object.entries({name:codeValue,code:codeValue,discount_type:"p",discount:String(discount),status:"1",applying_method:"CODE",apply_to:"all",free_shipping:"0",free_cod:"0",total:"0",uses_total:"1000",uses_customer:"100",date_start:now.toISOString().slice(0,10),date_end:end.toISOString().slice(0,10),conditions_criteria:"all",discount_strategy:"standard"}))form.set(key,value);
  ...
  if(body?.action==="category_assign"){
    ...
    const [productsCall,categoriesCall]=await Promise.all([fetch("https://api.zid.sa/v1/products/?page=1&page_size=100",{headers}),fetch("https://api.zid.sa/v1/managers/store/categories?page=1&per_page=100",{headers})]);
    const productsPayload=await productsCall.json().catch(()=>({})) as Record<string,unknown>,categoriesPayload=await categoriesCall.json().catch(()=>({})) as Record<string,unknown>;
    const products=(Array.isArray(productsPayload.results)?productsPayload.results:Array.isArray(productsPayload.products)?productsPayload.products:[]) as Array<Record<string,unknown>>,categories=(Array.isArray(categoriesPayload.categories)?categoriesPayload.categories:[]) as Array<Record<string,unknown>>;
    const nameOf=(value:unknown)=>typeof value==="object"&&value?String((value as Record<string,unknown>).en??(value as Record<string,unknown>).ar??""):String(value??"");
    const productMatches=products.filter(p=>nameOf(p.name).toLowerCase()===query||String(p.sku??"").toLowerCase()===query),categoryMatches=categories.filter(c=>nameOf(c.name).toLowerCase()===categoryName);
    if(productMatches.length!==1||categoryMatches.length!==1)return json({error:`Use exact names. Product matches: ${productMatches.length}; category matches: ${categoryMatches.length}.`},422);
    const productId=String(productMatches[0].id??""),categoryId=String(categoryMatches[0].id??"");const assign=await fetch(`https://api.zid.sa/v1/products/${encodeURIComponent(productId)}/categories/`,{method:"POST",headers:{...headers,"Content-Type":"application/json"},body:JSON.stringify({id:Number(categoryId)||categoryId})});
    ...
    return json({ok:true,confirmed:true,action_id:`PS-CATEGORY-${Date.now().toString(36).toUpperCase()}`,message:`${nameOf(productMatches[0].name)} was assigned to ${nameOf(categoryMatches[0].name)} in Zid.`,result});
  }
