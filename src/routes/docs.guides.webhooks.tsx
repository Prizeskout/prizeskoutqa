...
function H3({ children }: { children: React.ReactNode }) {
  return (
    <h3
      style={{ fontSize: 16, fontWeight: 600, color: "#1A1A18", marginTop: 24, marginBottom: 10 }}
    >
      {children}
    </h3>
  );
...
const HEADERS: { h: string; v: React.ReactNode }[] = [
  {
    ...
    v: (
      <>
        ...
        <Code>{"{timestamp}.{raw_body}"}</Code>. <Code>sha256</Code> is a legacy body-only HMAC kept
  ...
  },
  {
    ...
    v: (
      <>
        Unix epoch seconds when the signature was generated. Compare against your server clock to
        reject replays. Same value as the <Code>t=</Code> field inside the signature header.
      </>
    ),
  },
  {
    ...
    v: (
      <>
        ...
        <Code>pricing.recommendation_created</Code>, <Code>field_intel.observation_submitted</Code>.
      </>
    ),
  },
  {
    ...
    v: (
      <>
        Unique ID for this event. Retries reuse the same ID , store it and dedupe to make your
        handler idempotent.
      </>
    ),
  },
  {
    ...
    v: (
      <>
        <Code>1</Code> for the first delivery, <Code>2+</Code> for retries (exponential backoff up
        to the endpoint's <Code>max_attempts</Code>).
      </>
    ),
  },
];
      ...
      >
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 16, color: "#5A5A58", lineHeight: 1.65, marginBottom: 32 }}>
            ...
            HMAC-signed so you can trust the payload, and every failure is automatically retried
          ...
          <Callout tone="info" icon={<Webhook size={14} />} title="Signing secrets are shown once">
            When you create or rotate a webhook endpoint, the signing secret (<Code>whsec_…</Code>)
            is displayed exactly once in a copy-once dialog. Store it in your secret manager
            immediately , PrizeSkout only keeps the last four characters for the UI.
          </Callout>
          ...
          >
            ...
            <li style={{ marginBottom: 6 }}>
              ...
              <Code>|now − t| &gt; 300 seconds</Code>. This is the replay-protection window ,
              without it, an attacker who captures a valid request can resend it forever.
            </li>
            <li>
          ...
          <P>
            Use this to recompute the expected signature locally when investigating a failed
            delivery , paste the <Code>X-Webhook-Timestamp</Code>, raw body, and your signing
            secret:
          </P>
          ...
          <P>
            PrizeSkout retries any delivery that does not return a <Code>2xx</Code> within 10
            seconds. Retries use exponential backoff up to the endpoint's <Code>max_attempts</Code>{" "}
            setting. Because retries are possible, your handler <strong>must be idempotent</strong>:
          </P>
          <ul
            style={{
              paddingLeft: 22,
          ...
          >
            ...
            <li>
              Treat each event type as a state-transition signal, not a command. Re-applying the
              same <Code>competitor.price_changed</Code> event should leave your data in the same
              state.
            </li>
          </ul>
          <Callout tone="success" icon={<ShieldCheck size={14} />} title="Best practice">
            Acknowledge the webhook (return <Code>200</Code>) <em>before</em> doing slow work. Push
            the event onto a queue inside your handler and process it asynchronously. This keeps you
            under the 10-second timeout and avoids unnecessary retries.
          </Callout>
          ...
          <P>
            ...
            shared in a screen recording). In <strong>Dashboard → Webhooks</strong>, click the
            rotate icon on the endpoint , a new <Code>whsec_…</Code> is generated and shown once.
            The old secret stops working immediately, so deploy the new one to your server first.
          </P>
          <Callout
            tone="warning"
          ...
          >
            Unlike API keys, webhook secrets do not have an overlap window. The moment you rotate,
            any in-flight delivery still using the old secret will fail signature verification on
            your side. Plan rotations during a low-traffic window.
          </Callout>

          <H2 id="testing">Testing locally</H2>
          <P>
            Use a tunnel like <Code>ngrok</Code>, <Code>cloudflared tunnel</Code>, or your
            platform's preview URL to expose your local server to PrizeSkout. Point a webhook
            endpoint at the tunnel URL, then trigger a test delivery from{" "}
          ...
          <P>
            ...
            timestamp from <Code>X-Webhook-Timestamp</Code>, and the exact raw body , we return what
            we would have signed and (optionally) compare against what you received.
          </P>
  ...
  -d '{
    ...
    "receivedSignature": "t=1714060800,v1=...,sha256=..."
          ...
          <P>
            ...
            against the current server time, and , if you provided <Code>receivedSignature</Code> ,
            a constant-time comparison plus diagnostic notes explaining the most likely cause of any
            mismatch. Nothing from the request is logged or stored.
          </P>
