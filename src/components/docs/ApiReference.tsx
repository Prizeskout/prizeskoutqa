...
function buildCurl(spec: EndpointSpec, apiKey: string) {
  const path = spec.path.replace(
    /\{(\w+)\}/g,
    (_, n) => spec.pathParams?.find((p) => p.name === n)?.example ?? `{${n}}`,
  );
  const lines = [
    `curl -X ${spec.method} "${API_BASE_URL}${path}" \\`,
...
export function ApiReference() {
  ...
  const current = useMemo(() => {
    ...
    const endpoint =
      group.endpoints.find((e) => e.slug === selected.endpoint) ?? group.endpoints[0];
  ...
  const handleTryIt = async () => {
    if (!apiKey || apiKey === "sk_test_YOUR_KEY") {
      setTryResponse({
        status: 0,
        body: "Paste a test API key (sk_test_...) above to run this request.",
      });
      setTryStatus("error");
      return;
    }
    ...
    setTryResponse(null);
    try {
      const path = current.endpoint.path
        .replace(/\/v1\//, "/api/public/v1/")
        .replace(/\{(\w+)\}/g, (_, n) => {
  ...
  const codeText =
    tab === "curl" ? buildCurl(current.endpoint, apiKey) : buildJs(current.endpoint, apiKey);
      ...
      >
        <div style={{ minWidth: 520 }}>
          ...
          {fields.map((f, i) => (
            ...
            >
              <div
                style={{ fontFamily: FONT_MONO, fontSize: 12.5, color: "#1A1A18", fontWeight: 600 }}
              >
                {f.name}
              </div>
...
function EndpointDetail({ group, endpoint }: { group: GroupSpec; endpoint: EndpointSpec }) {
  return (
    <article>
      ...
      <p style={{ fontSize: 14.5, lineHeight: 1.65, color: "#3A3A38", marginBottom: 18 }}>
        {endpoint.summary}
      </p>
      {endpoint.description && (
        <p style={{ fontSize: 14, lineHeight: 1.65, color: "#3A3A38", marginBottom: 18 }}>
          {endpoint.description}
        </p>
      )}
      ...
      {endpoint.body && endpoint.body.length > 0 && (
        <FieldsTable title="Request body" fields={endpoint.body} />
      )}

      {/* Responses */}
      <section style={{ marginBottom: 28 }}>
        <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 12, color: "#1A1A18" }}>
          Responses
        </h3>
      ...
      {endpoint.notes && endpoint.notes.length > 0 && (
        <section style={{ marginBottom: 28 }}>
          <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 10, color: "#1A1A18" }}>
            Important notes
          </h3>
          <div
            style={{
    ...
    <div style={{ padding: "20px 18px" }}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ fontSize: 10.5, color: "#8A8A8A", marginTop: 6, lineHeight: 1.5 }}>
          Use a test key from Dashboard, API Keys. It stays in this page only and is cleared when
          you close the tab.
        </div>
      </div>
              ...
              style={{
                ...
                background:
                  tryResponse.status >= 200 && tryResponse.status < 300
                    ...
                    : "rgba(239, 68, 68, 0.15)",
                color:
                  tryResponse.status >= 200 && tryResponse.status < 300 ? "#22C55E" : "#EF4444",
                borderRadius: 4,
              }}
            >
