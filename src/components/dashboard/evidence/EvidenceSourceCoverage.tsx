...
export function EvidenceSourceCoverage(){
  ...
  return <section style={card}><div style={heading}><div><h2 style={h2}><Database size={18}/> Automatic evidence sources</h2><p style={muted}>Coverage shows only records PrizeSkout received. It does not guarantee the provider supplied every record.</p></div><span style={badge}>{sources.length} source{sources.length===1?"":"s"}</span></div>
    {loading?<div className="ps-evidence-loading" role="status" aria-live="polite"><Loader2 size={18} className="animate-spin"/><div><strong>Checking source coverage</strong><small>Reconciling provider health and delivery windows…</small></div></div>:error?<div style={warning}><AlertTriangle size={17}/>{error}</div>:sources.length===0?<div style={empty}><Database size={18}/><div><strong>No automatic sources connected</strong><small>Manual evidence remains available while you connect a provider.</small></div></div>:<div style={{display:"grid",gap:10}}>{sources.map(source=>{
      ...
      return <article key={source.id} style={sourceCard}><div style={heading}><div><strong>{title(source.provider)}</strong><div style={muted}>{title(source.connection_kind)} · {source.authorized?"Merchant authorized":"Authorization required"}</div></div><span style={{...healthBadge,background:healthy?"color-mix(in srgb, var(--green) 12%, var(--surface))":"color-mix(in srgb, var(--accent) 10%, var(--surface))",color:healthy?"var(--green)":"var(--accent)"}}><Icon size={13}/>{source.health.label}</span></div>
...
const card:CSSProperties={background:"var(--surface)",border:"1px solid var(--border)",borderRadius:12,padding:16,boxShadow:"var(--shadow)"};
const sourceCard:CSSProperties={border:"1px solid var(--border)",borderRadius:10,padding:14,display:"grid",gap:12,background:"var(--surface)"};
const heading:CSSProperties={display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12};
const h2:CSSProperties={display:"flex",alignItems:"center",gap:8,fontSize:15,margin:"0 0 4px",color:"var(--text)"};
const muted:CSSProperties={fontSize:12,color:"var(--muted)",margin:0};
const badge:CSSProperties={fontSize:11,fontWeight:700,color:"var(--muted)",background:"var(--surface2)",padding:"5px 8px",borderRadius:99};
...
const metricLabel:CSSProperties={display:"block",fontSize:10,textTransform:"uppercase",letterSpacing:'.04em',color:"var(--muted)",marginBottom:3};
const foot:CSSProperties={display:"flex",flexWrap:"wrap",justifyContent:"space-between",gap:8,fontSize:11,color:"var(--muted)"};
const tags:CSSProperties={display:"flex",flexWrap:"wrap",gap:6,fontSize:10,color:"var(--muted)"};
const empty:CSSProperties={display:"flex",alignItems:"center",gap:9,padding:"18px 4px",fontSize:13,color:"var(--muted)"};
const warning:CSSProperties={...empty,color:"var(--accent)",background:"color-mix(in srgb, var(--accent) 9%, var(--surface))",padding:12,borderRadius:8};
