...
type PromotionAction = {
  ...
  scenario_id: string;
  platform: string;
  action_type: "stop" | "reduce_discount" | "remove_item" | "adjust_discount";
...
export function PromotionProfitabilityWorkspace({
  ...
  const actionCall = async (payload: Record<string, unknown>) => {
    const response = await fetch("/api/channels/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          merchant_id: localStorage.getItem("ps_merchant_id") ?? "",
          access_code: localStorage.getItem("ps_access_code") ?? "",
          platform: "promotion_actions",
          ...payload,
        }),
      }),
      data = await response.json();
    if (!response.ok || !data.ok) throw new Error(data.error ?? "Promotion action request failed.");
    return data;
  };
  const loadActions = () =>
    actionCall({ action: "list" }).then((data) => setActions(data.actions ?? []));
  const load = () =>
    call({ action: "list" })
  ...
  const proposeAction = async (item: SavedPromotionScenario) => {
    const actionType = (await askText({ title: "Propose promotion action", input: { label: "Action", initialValue: "stop", placeholder: "stop, reduce_discount, remove_item, or adjust_discount" } })) as PromotionAction["action_type"] | null;
    if (
      !actionType ||
      !["stop", "reduce_discount", "remove_item", "adjust_discount"].includes(actionType)
    ) {
      if (actionType) setError("Choose stop, reduce_discount, remove_item, or adjust_discount.");
      return;
    }
    const target = (await askText({ title: "Target campaign", input: { label: "Exact partner campaign ID or item reference", initialValue: item.launch_manifest?.find((entry) => entry.partner_campaign_id)?.partner_campaign_id ?? "" } })) ?? "";
    if (!target.trim()) return;
    const requester = (await askText({ title: "Request owner", input: { label: "Requester name or email", initialValue: operationsReviewer } })) ?? "";
    if (!target.trim() || !requester.trim()) return;
    const discountValue = ["reduce_discount", "adjust_discount"].includes(actionType)
      ? await askText({ title: "Adjust discount", input: { label: "New discount percentage", initialValue: "10" } })
      : null;
    if (["reduce_discount", "adjust_discount"].includes(actionType) && discountValue === null) return;
    const value = discountValue === null ? null : Number(discountValue);
    ...
    try {
      await actionCall({
        ...
        requested_payload: value === null ? {} : { discount_pct: value },
      });
      await loadActions();
    } catch (error) {
      setError(error instanceof Error ? error.message : "Promotion action could not be proposed.");
    } finally {
      setBusy(false);
    }
  };
  const decideAction = async (item: PromotionAction, decision: "approved" | "rejected") => {
    const reviewer = (await askText({ title: decision === "approved" ? "Approve action" : "Reject action", input: { label: `${decision === "approved" ? "Approver" : "Reviewer"} name or email`, initialValue: financeReviewer }, confirmLabel: decision === "approved" ? "Approve" : "Reject" })) ?? "";
    if (!reviewer.trim()) return;
    setBusy(true);
    try {
      await actionCall({ action: "decide", id: item.id, decision, reviewer });
      await loadActions();
    } catch (error) {
      setError(error instanceof Error ? error.message : "Promotion action decision failed.");
    } finally {
      setBusy(false);
    }
  };
  const executeAction = async (item: PromotionAction) => {
    setBusy(true);
    try {
      await actionCall({ action: "execute", id: item.id });
      await loadActions();
    } catch (error) {
      setError(error instanceof Error ? error.message : "Promotion action could not be released.");
    } finally {
      setBusy(false);
    }
  };
  const confirmAction = async (item: PromotionAction) => {
    const reference = (await askText({ title: "Confirm partner action", input: { label: "Partner confirmation reference" }, confirmLabel: "Confirm action" })) ?? "";
    if (!reference.trim()) return;
    setBusy(true);
    try {
      await actionCall({ action: "confirm", id: item.id, partner_reference: reference });
      await loadActions();
    } catch (error) {
      setError(error instanceof Error ? error.message : "Promotion action could not be confirmed.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const result = useMemo(
    () => simulatePromotion(simulatedProducts, simulatedInputs),
  ...
  const sensitivity = useMemo(
    () =>
      [
        {
          label: "Downside",
          lift: Math.max(-100, simulatedInputs.expected_conversion_lift_pct - 15),
        },
        ...
        ...item,
        result: simulatePromotion(simulatedProducts, {
          ...simulatedInputs,
          expected_conversion_lift_pct: item.lift,
        }),
      })),
    [simulatedProducts, simulatedInputs],
  );
  ...
  const commercialInputsReady =
    [commission, platformFunding, vatOnFees, paymentFee, fixedOrderFee].every(
      (value) => value !== "",
    ) &&
  ...
  const saveDraft = async () => {
    if (
      ...
      !result.meets_margin_floor ||
      !result.beats_baseline
    ) {
      setError(
        "Run a current simulation backed by an approved contract and verified costs that meets the margin floor and beats the baseline before saving.",
      );
      return;
    }
    setBusy(true);
    ...
    try {
      await call({
        ...
        inputs: {
          ...simulationRequest.inputs,
          target_channels: targetChannels,
          simulation_signature: simulationRequest.signature,
  ...
  const resetSimulation = () => {
    setDiscount("20");
    setPlatformFunding(
      contract?.promotion_funding_platform_pct == null
        ? ""
        : String(contract.promotion_funding_platform_pct),
    );
    setCommission(
      contract?.commission_rate_pct == null ? "" : String(contract.commission_rate_pct),
    );
  ...
  const promotionCost = simulationRequest
    ? result.products.reduce((sum, product) => sum + product.merchant_discount, 0) *
      result.expected_orders
    : 0;
  const resultMargin =
    simulationRequest && result.campaign_contribution > 0
      ? Math.max(
          0,
          (result.campaign_contribution /
            Math.max(
              1,
              result.products
                .filter((product) => product.eligible)
                .reduce((sum, product) => sum + product.campaign_price, 0) * result.expected_orders,
            )) *
            100,
        )
      : null;
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 25, fontWeight: 900, marginTop: 5, letterSpacing: "-.03em" }}>
            Simulate impact before you spend
          </div>
          <div style={{ fontSize: 12.5, color: "var(--muted)", marginTop: 3 }}>
            Compare campaign economics, protect contribution margin, and prepare an approval-ready
            scenario.
          </div>
        </div>
        <span
      ...
      >
        ...
        >
          {[
            [
              ...
              simulationRequest ? money(result.campaign_contribution, currency) : "Not calculated",
              "After verified campaign costs",
            ],
            [
              "Promotion cost",
              simulationRequest ? money(promotionCost, currency) : "Not calculated",
              "Merchant-funded discount",
            ],
            [
              "Expected lift",
              `${n(lift) >= 0 ? "+" : ""}${lift}%`,
              "Merchant estimate, not a guarantee",
            ],
            [
              "Net profit impact",
              simulationRequest
                ? money(result.incremental_contribution, currency)
                : "Not calculated",
              simulationRequest
                ? result.beats_baseline
                  ? "Above baseline"
            ...
            <div
              ...
              style={{
                ...
                padding: "14px 15px",
                background: "var(--surface)",
                boxShadow: "0 8px 22px rgba(15,35,70,.04)",
              }}
            >
              <div
                style={{
        ...
        >
          ...
          >
            ...
            <div style={{ marginTop: 3, color: "var(--muted)", fontSize: 10.5 }}>
              Configure the commercial outcome you want to test.
            </div>
            <div style={{ display: "grid", gap: 10, marginTop: 14 }}>
              ...
              >
                <strong style={{ color: "var(--text)" }}>{selected.length} products</strong> ·{" "}
                {targetChannels
                  .map((channel) => channel.replaceAll("_", " ").toUpperCase())
                  .join(", ") || "No channel selected"}
              </div>
              <button
                type="button"
                disabled={busy || !selected.length || !commercialInputsReady}
                ...
                style={{
                  ...
                  fontFamily: "inherit",
                  fontWeight: 850,
                  cursor: !commercialInputsReady || !selected.length ? "not-allowed" : "pointer",
                  opacity: !commercialInputsReady || !selected.length ? 0.5 : 1,
                }}
              >
                <FlaskConical size={14} style={{ verticalAlign: "middle", marginInlineEnd: 6 }} />
                {simulationRequest ? "Recalculate simulation" : "Run simulation"}
              </button>
              <button
                type="button"
              ...
              >
                Reset to contract baseline
              </button>
            </div>
          </section>
          <div
            className="ps-promo-results"
            style={{ display: "grid", gridTemplateRows: "1fr auto", gap: 12, minWidth: 0 }}
          >
            <section
              className="ps-promo-comparison"
              style={{
                border: "1px solid var(--border)",
                borderRadius: 12,
                padding: 16,
            ...
            >
              ...
              >
                <div>
                  <div style={{ fontSize: 14, fontWeight: 900 }}>Scenario comparison</div>
                  <div style={{ marginTop: 3, color: "var(--muted)", fontSize: 10.5 }}>
                    Downside, expected, and upside demand outcomes
                  </div>
                </div>
                {simulationRequest && (
                  <span
                    style={{
                      color: simulationStale ? "#A16207" : "#087F5B",
                      fontSize: 10.5,
                      fontWeight: 800,
                    }}
                  >
                    {simulationStale ? "Results need recalculation" : "Current simulation"}
                  </span>
                )}
              </div>
              {simulationRequest ? (
                <div style={{ display: "grid", gap: 14, marginTop: 20 }}>
                  {sensitivity.map((scenario, index) => {
                    const max = Math.max(
                      1,
                      ...sensitivity.map((item) => Math.abs(item.result.campaign_contribution)),
                    );
                    const width = Math.max(
                      5,
                      (Math.abs(scenario.result.campaign_contribution) / max) * 100,
                    );
                    return (
                      <div
                        key={scenario.label}
                        style={{
                          display: "grid",
                          gridTemplateColumns: "70px minmax(0,1fr) 110px",
                          gap: 10,
                          alignItems: "center",
                        }}
                      >
                        <strong style={{ fontSize: 11 }}>{scenario.label}</strong>
                        <div
                          style={{
                            height: 22,
                            borderRadius: 6,
                        ...
                        >
                          <div
                            style={{
                              ...
                              height: "100%",
                              background:
                                index === 1 ? "#2563EB" : index === 2 ? "#EF681A" : "#94A3B8",
                              borderRadius: 6,
                            }}
                          />
                        </div>
                        <span style={{ textAlign: "right", fontSize: 11.5, fontWeight: 800 }}>
                          {money(scenario.result.campaign_contribution, currency)}
                        </span>
                      </div>
                    );
                  })}
                ...
                >
                  Run the scenario to compare outcomes.
                </div>
              )}
            </section>
            <section
              className="ps-promo-impact"
              style={{ border: "1px solid var(--border)", background: "var(--surface)" }}
            >
              <div style={{ fontSize: 12, fontWeight: 900 }}>Margin Impact Over Time</div>
              <div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 3 }}>
                Baseline, expected, and upside contribution
              </div>
              <div className="ps-promo-impact-chart">
                {sensitivity.map((scenario, index) => {
                  const maximum = Math.max(
                    1,
                    ...sensitivity.map((item) => Math.abs(item.result.incremental_contribution)),
                  );
                  const height = simulationRequest
                    ? Math.max(
                        5,
                        (Math.abs(scenario.result.incremental_contribution) / maximum) * 100,
                      )
                    : 5;
                  return (
                    <i
                      ...
                      style={{
                        height: `${height}%`,
                        background: ["#94a3b8", "#2563eb", "#f47320"][index],
                      }}
                    />
                  );
                })}
            ...
            <section
              className="ps-promo-recommendation"
              style={{
                border: `1px solid ${simulationRequest ? (result.beats_baseline && result.meets_margin_floor ? "#BBF7D0" : "#FED7AA") : "var(--border)"}`,
                borderRadius: 12,
                padding: 15,
                background: simulationRequest
                  ? result.beats_baseline && result.meets_margin_floor
                    ? "#F0FDF4"
                    : "#FFF7ED"
                  : "var(--surface)",
              }}
            >
              ...
              >
                <div>
                  <div style={{ fontSize: 12, fontWeight: 900 }}>Recommendation summary</div>
                  <strong style={{ display: "block", marginTop: 6, fontSize: 15 }}>
                    {!simulationRequest
                      ? "No scenario calculated"
                      : result.beats_baseline && result.meets_margin_floor
                        ? "This scenario is ready for merchant review"
                        : "Revise this scenario before approval"}
                  </strong>
                  <span
                    style={{ display: "block", marginTop: 4, color: "var(--muted)", fontSize: 11 }}
                  >
                    {!simulationRequest
                      ? "Complete the builder and run a simulation."
                      : `${result.eligible_products} eligible products · ${result.expected_orders} expected orders · ${resultMargin == null ? "margin unavailable" : `${resultMargin.toFixed(1)}% projected contribution margin`}`}
                  </span>
                </div>
                {simulationRequest && (
                  <span
                    style={{
                      padding: "5px 8px",
                      borderRadius: 999,
                      background: result.approval_ready ? "#DCFCE7" : "#FFEDD5",
                      color: result.approval_ready ? "#166534" : "#9A3412",
                      fontSize: 10.5,
                      fontWeight: 850,
                    }}
                  >
                    {result.approval_ready ? "Evidence ready" : "Planning estimate"}
                  </span>
                )}
              </div>
            </section>
        ...
        >
          ...
          >
            Products, channels, and contract assumptions
          </summary>
          <div style={{ padding: "4px 14px 14px" }}>
            ...
            >
              <MerchantField
                label="Campaign name"
                help="A private name for identifying this promotion. Customers will not see it."
                source="merchant"
              >
              ...
              <MerchantField
                label="Discount %"
                help="The percentage customers receive off the normal selling price."
                source="merchant"
                consequence="A larger discount can increase orders but reduce what you keep per order."
              >
                <input
                  type="number"
                  min={0}
              ...
              <MerchantField
                label="Platform funding %"
                help="How much of the customer discount the sales platform pays."
                source="contract"
                whereToFind="your campaign agreement"
              >
                <input
                  type="number"
                  min={0}
              ...
              <MerchantField
                label="Commission %"
                help="The percentage the platform charges on each campaign order."
                source="contract"
                whereToFind="your commercial agreement or latest payout statement"
              >
                <input
                  type="number"
                  min={0}
              ...
              <MerchantField
                label="VAT on fees %"
                help="VAT charged on the platform's commission and fees, not the customer's order total."
                source="contract"
              >
                <input
                  type="number"
              ...
              <MerchantField
                label="Payment fee %"
                help="The percentage deducted for processing the customer's payment."
                source="contract"
                whereToFind="your payment or platform agreement"
              >
                <input
                  type="number"
                  min={0}
              ...
              <MerchantField
                ...
                help={`Any fixed ${currency} amount charged on every campaign order.`}
                source="contract"
              >
                <input
                  type="number"
              ...
              <MerchantField
                ...
                help="The order amount the platform uses when calculating its commission."
                source="contract"
                whereToFind="the commission section of your agreement"
              >
                <select
                  style={input}
              ...
              <MerchantField
                label="Expected order lift %"
                help="Your estimated increase in orders while the promotion is running."
                source="estimate"
                consequence="PrizeSkout uses this estimate to project whether extra orders make up for the discount."
              >
                <input
                  type="number"
                  min={-100}
              ...
              <MerchantField
                ...
                help="How many orders you would normally expect during the same number of days without this promotion."
                source="estimate"
              >
                <input
                  type="number"
              ...
              <MerchantField
                label="Duration (days)"
                help="How many calendar days the promotion will run."
                source="merchant"
              >
                <input
                  type="number"
              ...
              <MerchantField
                ...
                help="The lowest profit margin you are willing to accept after all campaign costs."
                source="merchant"
                consequence="PrizeSkout warns you when a campaign would go below this protected margin."
              >
                <input
                  type="number"
                  min={0}
                ...
                {products.map((product) => (
                  <label
                    ...
                    style={{
                      ...
                      fontSize: 11.5,
                      cursor: "pointer",
                      background: selected.includes(product.sku)
                        ? "var(--surface2)"
                        : "transparent",
                    }}
                  >
                ...
                {["zid", "salla", "talabat", "jahez", "keeta", "in_store"].map((channel) => (
                  <label
                    ...
                    style={{
                      ...
                      fontSize: 11.5,
                      cursor: "pointer",
                      background: targetChannels.includes(channel)
                        ? "var(--surface2)"
                        : "transparent",
                    }}
                  >
            ...
            >
              {!!validationErrors.length && (
                <div style={{ color: "#B42318", fontSize: 12, marginBottom: 10 }}>
                  <strong>Fix these inputs before simulating:</strong>
              ...
              {simulationStale && (
                <div style={{ color: "#A16207", fontSize: 12, marginBottom: 10 }}>
                  <strong>Results out of date.</strong> An assumption, product, or channel changed.
                  Recalculate before saving or approval.
                </div>
              )}
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                ...
                <span style={{ fontSize: 11.5, color: "var(--muted)" }}>
                  {simulationRequest && !simulationStale
                    ? `Calculated ${new Date(simulationRequest.calculatedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`
                    : "No current result"}
                </span>
              </div>
            </div>
        ...
        {simulationRequest ? (
          <>
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          style={{
                            color:
                              item.result.incremental_contribution >= 0 ? "#087F5B" : "#B42318",
                          }}
                        >
                  ...
                  {result.products.map((product) => (
                    <tr key={product.sku}>
                      {[
                        ...
                        product.expected_margin_pct == null
                          ? "—"
                          : `${product.expected_margin_pct}%`,
            ...
            <div style={{ fontSize: 10.5, color: "var(--muted)" }}>
              * Product cost shows its evidence basis. Verified values come from the connected
              catalogue economics snapshot; inferred values remain clearly labelled. No campaign is
              launched from this workspace.
            </div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", fontSize: 11.5 }}>
              <strong style={{ color: result.beats_baseline ? "#087F5B" : "#B42318" }}>
                {result.beats_baseline ? "Beats baseline" : "Below baseline"}
              </strong>
              <strong style={{ color: result.meets_margin_floor ? "#087F5B" : "#B42318" }}>
                {result.meets_margin_floor
                  ? "Every product meets the floor"
                  : "At least one product is below the floor"}
              </strong>
              <strong style={{ color: result.approval_ready ? "#087F5B" : "#A16207" }}>
                {result.approval_ready ? "Evidence ready for review" : "Planning estimate only"}
              </strong>
            </div>
          </>
        ) : (
          ...
          >
            Complete the assumptions and click <strong>Run simulation</strong>. PrizeSkout will not
            present calculated campaign results before validation.
          </div>
        )}
        <div style={{ display: "flex", gap: 9, alignItems: "center", flexWrap: "wrap" }}>
          {!commercialInputsReady && (
            <div style={{ fontSize: 11.5, color: "#A16207", alignSelf: "center" }}>
              Enter the commission, commission basis, funding split, VAT, payment fee and fixed fee
              before saving. Enter a reviewed zero when the agreement confirms no charge; PrizeSkout
              will not treat a blank as zero.
            </div>
          )}
          <button
            disabled={
                  ...
                  >
                    ...
                    {["approved", "ready_to_launch", "running"].includes(item.status) && (
                      <button
                        type="button"
                        disabled={busy}
                        onClick={() => void proposeAction(item)}
                        style={{
        ...
        {!!actions.length && (
          ...
          >
            <h3 style={{ margin: "0 0 4px", fontSize: 13 }}>Approved promotion action queue</h3>
            <p style={{ margin: "0 0 10px", fontSize: 10.5, color: "var(--muted)" }}>
              No change is released until a different reviewer explicitly approves it. Connected
              partners can pull approved actions through the PrizeSkout API.
            </p>
            <div style={{ display: "grid", gap: 7 }}>
              {actions.map((item) => (
                ...
                >
                  <span>
                    <strong>{item.action_type.replaceAll("_", " ").toUpperCase()}</strong> ·{" "}
                    {item.platform.toUpperCase()} · {item.target_reference}
                    <span style={{ display: "block", marginTop: 3, color: "var(--muted)" }}>
                      Requested by {item.requested_by} · {item.status.replaceAll("_", " ")}
                      {item.delivery_mode ? ` · ${item.delivery_mode.replaceAll("_", " ")}` : ""}
                    </span>
                  </span>
                  <div style={{ display: "flex", gap: 6 }}>
                    {item.status === "pending_approval" && (
                      <>
                        <button
                          disabled={busy}
                          onClick={() => void decideAction(item, "approved")}
                          style={{ ...input, width: "auto", padding: "6px 8px", color: "#087F5B" }}
                        >
                          Approve
                        </button>
                        <button
                          disabled={busy}
                          onClick={() => void decideAction(item, "rejected")}
                          style={{ ...input, width: "auto", padding: "6px 8px", color: "#B42318" }}
                        >
                          Reject
                        </button>
                      </>
                    ...
                    {item.status === "approved" && (
                      <button
                        ...
                        style={{ ...input, width: "auto", padding: "6px 8px", color: "#087F5B" }}
                      >
                        Release approved action
                      </button>
                    )}
                    {item.status === "awaiting_partner" && item.delivery_mode === "manual" && (
                      <button
                        disabled={busy}
                    ...
                    {item.status === "confirmed" && (
                      <span style={{ color: "#087F5B", fontWeight: 800 }}>
                        Confirmed · {item.partner_reference}
                      </span>
                    )}
                  </div>
                </div>
