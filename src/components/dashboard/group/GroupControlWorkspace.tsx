...
export function GroupControlWorkspace({contract,currency,productCount}:{contract:ContractTerm|null;currency:string;productCount:number}){
  ...
  const approve=async(role:"finance"|"operations")=>{if(!reviewer.trim()){setError("Enter the reviewer’s name first.");return;}setBusy(true);try{const data=await call({action:"approve",approval_role:role,reviewer});apply(data.group);}catch(err){setError(err instanceof Error?err.message:"Could not record approval.");}finally{setBusy(false);}};
  ...
  return <section style={{border:"1px solid var(--border)",borderRadius:14,overflow:"hidden",background:"var(--surface)"}}>
    ...
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:17}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"end",flexWrap:"wrap"}}><button disabled={busy||!groupName.trim()} onClick={save} style={{border:0,borderRadius:8,padding:"9px 12px",background:"#14213D",color:"#fff",fontFamily:"inherit",fontWeight:800,display:"flex",gap:6}}><Save size={14}/>Save hierarchy</button><label style={{fontSize:10.5,fontWeight:800}}>Reviewer<input style={{...input,width:210,marginLeft:6}} value={reviewer} onChange={e=>setReviewer(e.target.value)}/></label><button disabled={busy||!saved} onClick={()=>approve("finance")} style={{border:"1px solid #087F5B",borderRadius:8,padding:"8px 10px",background:"transparent",color:"#087F5B",fontFamily:"inherit",fontWeight:800}}>Finance approve</button><button disabled={busy||!saved} onClick={()=>approve("operations")} style={{border:"1px solid #087F5B",borderRadius:8,padding:"8px 10px",background:"transparent",color:"#087F5B",fontFamily:"inherit",fontWeight:800}}>Operations approve</button>{saved?.finance_approved_at&&saved.operations_approved_at&&<span style={{fontSize:11,color:"#087F5B",fontWeight:900,display:"flex",gap:5}}><CheckCircle2 size={14}/>Dual approval complete</span>}</div>
