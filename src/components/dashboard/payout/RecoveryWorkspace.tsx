...
export function RecoveryWorkspace({findings,contract,currency,orderCount}:{findings:Finding[];contract:ContractTerm|null;currency:string;orderCount:number}) {
  ...
  return <section style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",flexWrap:"wrap"}}><div><div style={{fontSize:17,fontWeight:900}}>Claims and Recovery Workspace</div><div style={{fontSize:11.5,color:"var(--muted)",marginTop:3}}>Prepare, track and evidence claims. PrizeSkout does not claim external submission unless you record it.</div></div><span style={{fontSize:11.5,fontWeight:800}}>{cases.length} cases · {money(cases.reduce((sum,item)=>sum+item.recovered_amount,0),currency)} recovered</span></div>
