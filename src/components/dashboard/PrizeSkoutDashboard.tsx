...
export function PrizeSkoutDashboard() {
  ...
  const prepareCopilotOperation = async (operation: Record<string,unknown>) => {
    ...
    try {
      ...
      if(["change_order_status","create_product_draft"].includes(op)){
        ...
        const lines=preview.products.map(product=>`${product.name} (${product.sku}): ${product.changes.map(change=>`${change.field} ${JSON.stringify(change.before)} → ${JSON.stringify(change.after)}`).join(", ")}`);
        setCpOperationMessage(`Connected store: “${preview.store.title}” (ID ${preview.store.id}). ${preview.count} exact product match${preview.count===1?"":"es"}. ${lines.join(" ")} Approval expires at ${new Date(preview.expires_at).toLocaleTimeString()}. ${preview.warning}`);
  ...
  const executeCopilotStoreWrite=async()=>{
    ...
    try{
      ...
      setCpStoreActionResult({confirmed:Boolean(result.confirmed??result.ok),action_id:result.action_id??(operation==="seed_test_store"?"PS-ZID-SEED":"PS-ZID-CLEANUP"),message:result.message??"Completed"});
              ...
              {cpPhase === "result" && cpObj?._type === "operation" && (
                <div style={{ border:"1px solid var(--border)", borderRadius:14, overflow:"hidden", animation:"pk-in .35s ease" }}>
                  ...
                  <div style={{ padding:"18px 19px", display:"flex", flexDirection:"column", gap:14 }}>
                    ...
                    {["change_order_status","create_product_draft","product_change","seed_test_store","cleanup_test_store"].includes(String(cpObj.operation))&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:14,flexWrap:"wrap",border:"1px solid color-mix(in srgb,#F59E0B 35%,var(--border))",borderRadius:11,padding:"13px 14px",background:"color-mix(in srgb,#F59E0B 8%,var(--surface))"}}><div style={{fontSize:13.5,maxWidth:720}}><strong>Sensitive store action.</strong> {String(cpObj.operation)==="change_order_status"?`Order ${String(cpObj.order_id)} will move to ${String(cpObj.order_status)}.`:String(cpObj.operation)==="create_product_draft"?`An unpublished draft ${String(cpObj.product_name)} (${String(cpObj.product_sku)}) will be created at SAR ${Number(cpObj.product_price).toLocaleString()}.`:String(cpObj.operation)==="product_change"?String(cpObj.product_mode)==="delete"?"The exact product shown above will be permanently deleted; this cannot be rolled back.":"Only the exact field changes shown above will be sent to Zid.":String(cpObj.operation)==="cleanup_test_store"?"Only PS-ZID test products will be unpublished and PSMARGIN20/PSSAFE5 removed.":"Nine idempotent PS-ZID fixtures and two review coupons will be prepared in the confirmed Zid test store."} Nothing happens until you approve.</div><button onClick={executeCopilotStoreWrite} disabled={cpOperationStatus==="publishing"||cpOperationStatus==="complete"} style={{border:0,borderRadius:9,padding:"10px 14px",background:cpOperationStatus==="complete"?GN:OG,color:"white",fontFamily:"inherit",fontWeight:800,cursor:cpOperationStatus==="publishing"?"wait":"pointer"}}>{cpOperationStatus==="publishing"?"Executing...":cpOperationStatus==="complete"?"Confirmed in Zid":"Approve exact action"}</button></div>}
