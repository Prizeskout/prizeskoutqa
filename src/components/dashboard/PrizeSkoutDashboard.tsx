...
export function PrizeSkoutDashboard() {
  ...
  const [cpStoreActionResult,setCpStoreActionResult]=useState<{confirmed:boolean;action_id:string;message:string;products?:Array<{id:string;sku:string;source_sku?:string;status:string;after?:{id:string;sku:string;name:string;price:number|null;cost:number|null;quantity:number|null;is_infinite:boolean;is_published:boolean;is_draft:boolean}}>}|null>(null);
  ...
  const prepareCopilotOperation = async (operation: Record<string,unknown>) => {
    ...
    try {
      ...
      if(["change_order_status","create_product_draft"].includes(op)){
        ...
        const data=await response.json() as {preview?:{store:{id:string;title:string;test_store_confirmed:boolean};source_product:{name:string;sku:string}|null;products:Array<{sku:string;name:string;price:number;cost:number|null;quantity:number;note:string}>;coupons:Array<{code:string;discount:number;note:string}>;existing:{products:string[];coupons:string[]};ready:boolean;blockers:string[];warnings:string[]};error?:string};
        if(!response.ok||!data.preview)throw new Error(data.error??"Could not inspect the Zid test store.");
        const p=data.preview;if(!p.ready)throw new Error(p.blockers.join(" "));
        setCpObj(current=>current?{...current,test_store_id:p.store.id,test_store_title:p.store.title,confirm_test_store:true}:current);
        const confirmation=p.store.test_store_confirmed?"Zid identifies it as a test store.":"Zid does not label it as a test store; approving explicitly confirms this exact store is disposable test data.";
        ...
        setCpObj(current=>current?{...current,approval_token:preview.approval_token,product_change_preview:preview}:current);
        const first=preview.products[0],newProduct=first?.changes.find(change=>change.field==="new product")?.after;
        setCpOperationMessage(String(operation.product_mode)==="duplicate"?`I found ${first?.name??"the product"}. I’ll create ${String(newProduct??operation.new_product_name??"the new product")} in ${preview.store.title}. The original will stay unchanged.`:`I found ${preview.count} matching product${preview.count===1?"":"s"} in ${preview.store.title}. Review the change below, then approve when it looks right.`);
  ...
  const executeCopilotStoreWrite=async()=>{
    ...
    if(!["change_order_status","create_product_draft","product_change","seed_test_store","cleanup_test_store"].includes(operation))return;
  ...
  const prepareCreatedProductPublish=async(sku:string)=>{
    const merchantId=localStorage.getItem("ps_merchant_id")??"",accessCode=localStorage.getItem("ps_access_code")??"";setCpOperationStatus("running");
    try{
      const response=await fetch("/api/copilot/store",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({merchant_id:merchantId,access_code:accessCode,action:"preview_product_change",product_request:{mode:"publish",sku,scope:"single",changes:{}}})});
      const data=await response.json() as {preview?:{approval_token:string;store:{title:string};products:Array<{name:string;sku:string;changes:Array<{field:string;before:unknown;after:unknown}>}>};error?:string};
      if(!response.ok||!data.preview)throw new Error(data.error??"Could not prepare publication.");
      setCpObj(current=>current?{...current,operation:"product_change",product_mode:"publish",approval_token:data.preview!.approval_token,product_change_preview:data.preview,summary:`Publish ${data.preview!.products[0]?.name??sku}`}:current);setCpStoreActionResult(null);setCpOperationMessage(`I found ${data.preview.products[0]?.name??sku}. It is ready to be published to your Zid storefront.`);setCpOperationStatus("ready");
    }catch(error){setCpOperationStatus("failed");setCpOperationMessage(error instanceof Error?error.message:"Could not prepare publication.");}
  };
              ...
              {cpPhase === "result" && cpObj?._type === "operation" && (
                <div style={{ border:"1px solid var(--border)", borderRadius:14, overflow:"hidden", animation:"pk-in .35s ease" }}>
                  <div style={{ padding:"17px 19px", background:"var(--surface2)", display:"flex", justifyContent:"space-between", gap:12, flexWrap:"wrap", alignItems:"center" }}>
                    <div>
                      <div style={{ fontSize:11, color:"var(--muted)", textTransform:"uppercase", letterSpacing:".08em" }}>{String(cpObj.operation)==="product_change"?(cpOperationStatus==="complete"?"Product updated":"Review before I continue"):"Operational plan"}</div>
                      <div style={{ fontSize:17, fontWeight:800, marginTop:4 }}>{String(cpObj.operation)==="product_change"?(cpOperationStatus==="complete"?"Here’s the product now":cpPrompt):String(cpObj.summary ?? cpPrompt)}</div>
                    </div>
