...
const eventTime = (value: string | null | undefined) => {
  ...
  const timestamp = /^\d{4}-\d{2}-\d{2}$/.test(candidate)
...
function finishEvent(
  row: Omit<NormalizedCommerceEvent, "event_fingerprint">,
...
export function buildNormalizedCommerceEvents(input: BuildInput) {
  ...
  const base = {
    ...
    channel,
    branch_external_id: clean(input.result.branch_external_id),
    discount_amount: null as null,
    tax_amount: null as null,
    normalization_version: NORMALIZED_COMMERCE_VERSION,
  };
  ...
  for (const row of input.result.transaction_rows ?? []) {
    ...
    if (!orderId || !finite(row.gross_sales) || !finite(row.refund_amount)) {
      limitations.push(
        "At least one transaction row was skipped because its order ID or amount was invalid.",
      );
      continue;
    }
    ...
    events.push(
      finishEvent({
        ...
        normalized_payload: {
          ...
          claims_ready: row.claims_ready ?? false,
          refund_amount: money(row.refund_amount),
        },
        evidence_strength: row.claims_ready ? "strong" : "partial",
        limitations: row.claims_ready
          ? rowLimitations
          : [
              ...rowLimitations,
              "This order is not yet supported by complete claim-ready evidence.",
            ],
      }),
    );
  ...
  for (const [index, row] of (input.result.settlement_rows ?? []).entries()) {
    ...
    if (!reference || !finite(row.amount)) {
      limitations.push(
        "At least one settlement row was skipped because its reference or amount was invalid.",
      );
      continue;
    }
    const orderId = clean(row.order_id);
    const rowLimitations = orderId
      ? []
      : ["This settlement amount is not allocated to a specific order."];
    events.push(
      finishEvent({
        ...base,
  ...
  if (input.documentKind === "settlement_report" && finite(input.result.expected_payout)) {
    ...
    events.push(
      finishEvent({
        ...
        fee_amount: finite(input.result.commission_amount)
          ? money(input.result.commission_amount)
          : null,
        net_amount: money(input.result.expected_payout),
        normalized_payload: {
          period_start: input.result.period_start ?? null,
          period_end: input.result.period_end ?? null,
          additional_charges: input.result.additional_charges ?? null,
          additional_income: input.result.additional_income ?? null,
        ...
        limitations: reference
          ? []
          : [
              "The payout total has no settlement reference and can only be matched by platform and period.",
            ],
      }),
    );
  }

  const singleOrder = (input.result.order_references ?? [])
    .map(clean)
    .filter((value): value is string => Boolean(value))
    .filter((value, index, all) => all.indexOf(value) === index);
  const singleSettlement = (input.result.settlement_references ?? [])
    .map(clean)
    .filter((value): value is string => Boolean(value))
    .filter((value, index, all) => all.indexOf(value) === index);
  const orderId = singleOrder.length === 1 ? singleOrder[0] : null;
  const settlementReference =
    singleSettlement.length === 1 ? singleSettlement[0] : clean(input.result.settlement_reference);
  const occurredAt = eventTime(input.result.period_end);

  if (finite(input.result.cancellation_amount))
    events.push(
      finishEvent({
        ...base,
        external_event_id: `cancellation:${orderId ?? input.evidenceItemId}`,
        event_kind: "cancellation",
        order_external_id: orderId,
        settlement_reference: settlementReference,
        occurred_at: occurredAt,
        currency,
        gross_amount: money(input.result.cancellation_amount),
        fee_amount: null,
        net_amount: -money(input.result.cancellation_amount),
        normalized_payload: { allocated_to_order: Boolean(orderId) },
        evidence_strength: orderId ? "strong" : "partial",
        limitations: orderId
          ? []
          : ["The cancellation amount is not allocated to one specific order."],
      }),
    );

  if (
    ["credit_note", "adjustment_notice"].includes(input.documentKind) &&
    finite(input.result.adjustment_amount)
  ) {
    const direction = input.result.adjustment_direction;
    const signed =
      direction === "credit"
        ? money(input.result.adjustment_amount)
        : direction === "debit"
          ? -money(input.result.adjustment_amount)
          : null;
    events.push(
      finishEvent({
        ...base,
        external_event_id: `adjustment:${input.result.confirmation_reference ?? input.evidenceItemId}`,
        event_kind: "commercial_adjustment",
        order_external_id: orderId,
        settlement_reference: settlementReference,
        occurred_at: occurredAt,
        currency,
        gross_amount: money(input.result.adjustment_amount),
        fee_amount: null,
        net_amount: signed,
        normalized_payload: {
          direction,
          reason: input.result.adjustment_reason ?? null,
          allocated_to_order: Boolean(orderId),
        },
        evidence_strength:
          signed !== null && (orderId || settlementReference) ? "strong" : "partial",
        limitations: [
          ...(signed === null
            ? ["The document does not establish whether the adjustment is a credit or debit."]
            : []),
          ...(!orderId && !settlementReference
            ? ["The adjustment is not allocated to a specific order or settlement."]
            : []),
        ],
      }),
    );
  ...
  if (
    input.documentKind === "promotion_confirmation" &&
    [
      input.result.promotion_discount_total,
      input.result.platform_funding_amount,
      input.result.merchant_funding_amount,
    ].some(finite)
  )
    events.push(
      finishEvent({
        ...base,
        external_event_id: `promotion:${input.result.confirmation_reference ?? input.evidenceItemId}`,
        event_kind: "promotion_term",
        order_external_id: orderId,
        settlement_reference: settlementReference,
        occurred_at: occurredAt,
        currency,
        gross_amount: null,
        discount_amount: finite(input.result.promotion_discount_total)
          ? money(input.result.promotion_discount_total)
          : null,
        fee_amount: null,
        net_amount: null,
        normalized_payload: {
          platform_funding_amount: finite(input.result.platform_funding_amount)
            ? money(input.result.platform_funding_amount)
            : null,
          merchant_funding_amount: finite(input.result.merchant_funding_amount)
            ? money(input.result.merchant_funding_amount)
            : null,
          allocated_to_order: Boolean(orderId),
        },
        evidence_strength: orderId ? "strong" : "partial",
        limitations: orderId
          ? []
          : ["Promotion funding is retained as batch evidence and is not allocated to an order."],
      }),
    );

  if (input.documentKind === "merchant_confirmation" && finite(input.result.received_amount))
    events.push(
      finishEvent({
        ...base,
        external_event_id: `receipt:${input.result.confirmation_reference ?? input.evidenceItemId}`,
        ...
        gross_amount: null,
        fee_amount: null,
        net_amount: money(input.result.received_amount),
        normalized_payload: {
          confirmation_reference: clean(input.result.confirmation_reference),
          merchant_confirmed: true,
        },
        evidence_strength:
          settlementReference && clean(input.result.confirmation_reference)
            ? "confirmed"
            : "partial",
        limitations: settlementReference
          ? []
          : [
              "The payment confirmation has no settlement reference and cannot be matched automatically.",
            ],
      }),
    );

  if (
    (input.result.daily_rows?.length ?? 0) > 0 &&
    (input.result.transaction_rows?.length ?? 0) === 0
  ) {
    limitations.push(
      "Daily totals were retained as summary evidence and were not converted into fictional order records.",
    );
  }
  if (events.length === 0)
    limitations.push(
      "This document does not contain enough item-level detail for normalized commerce events.",
    );
...
export async function persistNormalizedCommerceEvents(input: BuildInput) {
  ...
  const { data: prior, error: priorError } = await db
    ...
    .eq("processor_version", NORMALIZED_COMMERCE_VERSION)
    .order("attempt_number", { ascending: false })
    .limit(1)
    .maybeSingle();
  if (priorError)
    throw new Error(priorError.message ?? "Normalization attempt history could not be read.");
  await appendEvidenceProcessingAttempt({
    evidenceItemId: input.evidenceItemId,
    accountId: input.accountId,
    processorVersion: NORMALIZED_COMMERCE_VERSION,
    attemptNumber: Number(prior?.attempt_number ?? 0) + 1,
    state: built.events.length ? "normalized" : "needs_review",
    detectedDocumentKind: input.documentKind,
    extractionSummary: {
      event_count: built.events.length,
      event_kinds: [...new Set(built.events.map((event) => event.event_kind))],
    },
