export type PriceChannel="in_store"|"zid"|"salla"|"foodics"|"talabat"|"snoonu"|"jahez"|"keeta";
...
export type ChannelPriceRow={
  ...
  source_platform:string;ingest_event_id:string;
};
...
export function planChannelPrices(products:ChannelPriceProduct[],channels:ChannelEconomics[],approvalAbovePct:number,maxChangePct:number):ChannelPriceRow[]{
  return products.flatMap(product=>channels.map(config=>{
    ...
    if(denominator<=0)return {sku:product.sku,name:product.name,channel:config.channel,current_price:product.current_price,planned_price:product.current_price,inferred_cost:round(cost),contribution:null,margin_pct:null,change_pct:null,consumer_difference:null,status:"excluded",reason:"Fees and margin target leave no feasible selling price.",source_platform:product.source_platform,ingest_event_id:product.ingest_event_id} satisfies ChannelPriceRow;
    ...
    return {
      ...
      source_platform:product.source_platform,ingest_event_id:product.ingest_event_id,
    };
  }));
}
