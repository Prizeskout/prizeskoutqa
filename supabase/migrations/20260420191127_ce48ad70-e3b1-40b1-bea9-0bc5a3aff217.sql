-- ROI model: per-tenant tunable parameters per category
create table public.roi_model_categories (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null,
  category text not null,
  elasticity numeric not null,
  baseline_daily_orders numeric not null,
  avg_order_value numeric not null,
  base_margin numeric not null,
  cannibalization_base numeric not null,
...
alter table public.roi_model_categories enable row level security;

create policy "Users can view own roi model"
  on public.roi_model_categories for select
  to authenticated using (auth.uid() = user_id);
create policy "Users can insert own roi model"
  on public.roi_model_categories for insert
  to authenticated with check (auth.uid() = user_id);
create policy "Users can update own roi model"
  on public.roi_model_categories for update
  to authenticated using (auth.uid() = user_id);
create policy "Users can delete own roi model"
  on public.roi_model_categories for delete
  to authenticated using (auth.uid() = user_id);

create trigger roi_model_categories_set_updated_at
  before update on public.roi_model_categories
  for each row execute function public.set_updated_at();

-- Persisted simulator history (last N per user)
create table public.promotions_scenarios (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null,
  category text not null,
  depth text not null,
  ...
  gmv_uplift numeric not null,
  incremental_orders integer not null,
  cannibalization_pct numeric not null,
...
alter table public.promotions_scenarios enable row level security;

create policy "Users can view own scenarios"
  on public.promotions_scenarios for select
  to authenticated using (auth.uid() = user_id);
create policy "Users can insert own scenarios"
  on public.promotions_scenarios for insert
  to authenticated with check (auth.uid() = user_id);
create policy "Users can update own scenarios"
  on public.promotions_scenarios for update
  to authenticated using (auth.uid() = user_id);
create policy "Users can delete own scenarios"
  on public.promotions_scenarios for delete
  to authenticated using (auth.uid() = user_id);

create index promotions_scenarios_user_simulated_at
  on public.promotions_scenarios (user_id, simulated_at desc);

-- Cap each user's history at 10 rows (trim oldest non-baseline first, then oldest overall)
create or replace function public.trim_promotions_scenarios()
returns trigger
language plpgsql
security definer
...
as $$
begin
  delete from public.promotions_scenarios
  where id in (
    select id from public.promotions_scenarios
...
create trigger promotions_scenarios_trim
  after insert on public.promotions_scenarios
  for each row execute function public.trim_promotions_scenarios();

-- Seed function: per-category ROI model defaults for one user
create or replace function public.seed_roi_model_for_user(uid uuid)
returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  insert into public.roi_model_categories
    (user_id, category, elasticity, baseline_daily_orders, avg_order_value, base_margin, cannibalization_base, position)
  values
    (uid, 'Electronics', 1.8, 220,  480, 0.18, 0.12, 0),
    (uid, 'Grocery',     0.9, 1800, 65,  0.14, 0.22, 1),
    (uid, 'Fashion',     2.1, 340,  180, 0.42, 0.18, 2),
    (uid, 'Home',        1.4, 160,  240, 0.28, 0.15, 3),
    (uid, 'Beauty',      1.6, 410,  120, 0.38, 0.20, 4)
  on conflict (user_id, category) do nothing;
end;
$$;

-- Wire into new-user trigger by extending handle_new_user
...
-- Backfill ROI model for existing users
do $$
declare
  pid uuid;
begin
  for pid in select id from public.profiles loop
    perform public.seed_roi_model_for_user(pid);
  end loop;
end $$;