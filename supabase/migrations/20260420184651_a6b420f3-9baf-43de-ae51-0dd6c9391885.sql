...
create policy "Users can view own category perf" on public.category_performance for select to authenticated using (auth.uid() = user_id);
create policy "Users can insert own category perf" on public.category_performance for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can update own category perf" on public.category_performance for update to authenticated using (auth.uid() = user_id);
create policy "Users can delete own category perf" on public.category_performance for delete to authenticated using (auth.uid() = user_id);

-- trending_products
create policy "Users can view own trending products" on public.trending_products for select to authenticated using (auth.uid() = user_id);
create policy "Users can insert own trending products" on public.trending_products for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can update own trending products" on public.trending_products for update to authenticated using (auth.uid() = user_id);
create policy "Users can delete own trending products" on public.trending_products for delete to authenticated using (auth.uid() = user_id);

-- assortment_gaps
create policy "Users can view own assortment gaps" on public.assortment_gaps for select to authenticated using (auth.uid() = user_id);
create policy "Users can insert own assortment gaps" on public.assortment_gaps for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can update own assortment gaps" on public.assortment_gaps for update to authenticated using (auth.uid() = user_id);
create policy "Users can delete own assortment gaps" on public.assortment_gaps for delete to authenticated using (auth.uid() = user_id);

-- cross_border_radar
create policy "Users can view own cross border" on public.cross_border_radar for select to authenticated using (auth.uid() = user_id);
create policy "Users can insert own cross border" on public.cross_border_radar for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can update own cross border" on public.cross_border_radar for update to authenticated using (auth.uid() = user_id);
create policy "Users can delete own cross border" on public.cross_border_radar for delete to authenticated using (auth.uid() = user_id);
...
create trigger set_category_performance_updated_at before update on public.category_performance for each row execute function public.set_updated_at();
create trigger set_trending_products_updated_at before update on public.trending_products for each row execute function public.set_updated_at();
create trigger set_assortment_gaps_updated_at before update on public.assortment_gaps for each row execute function public.set_updated_at();
create trigger set_cross_border_radar_updated_at before update on public.cross_border_radar for each row execute function public.set_updated_at();

-- ========== SEED FUNCTION ==========

create or replace function public.seed_market_for_user(uid uuid)
returns void
language plpgsql
security definer
...
begin
  -- market metrics
  insert into public.market_metrics (user_id, slug, label, value, value_color, footer_text, footer_color, position) values
    (uid, 'categories_tracked', 'Categories tracked', '12', '#1A1A18', 'across online and in-store', '#6B6B6B', 0),
    (uid, 'fastest_growing', 'Fastest growing', 'Grocery', '#22C55E', '+22% this month', '#22C55E', 1),
    (uid, 'assortment_gaps', 'Assortment gaps found', '14', '#1A1A18', 'products you are missing', '#6B6B6B', 2),
    (uid, 'cross_border_threats', 'Cross-border threats', '6', '#EF4444', 'products undercut by international sellers', '#6B6B6B', 3);

  -- category performance
  insert into public.category_performance (user_id, category, growth, avg_discount, volatility, volatility_pct, top_mover, market_position, direction, position) values
    (uid, 'Electronics', 14, 8,  'Medium', 50, 'Samsung Galaxy S24 series', '3rd / 6', 'up',   0),
    (uid, 'Grocery',     22, 12, 'Low',    25, 'Ramadan bundles',           '2nd / 4', 'up',   1),
    (uid, 'Fashion',      6, 18, 'High',   80, 'Nike Air Max series',       '4th / 5', 'flat', 2),
    (uid, 'Home',        -3, 15, 'Medium', 55, 'Dyson products',            '5th / 6', 'down', 3),
    (uid, 'Beauty',      19, 10, 'Medium', 45, 'The Ordinary serums',       '4th / 5', 'up',   4),
    (uid, 'Baby & Kids', 11,  7, 'Low',    20, 'Pampers bundles',           '2nd / 3', 'up',   5),
    (uid, 'Sports',       8, 14, 'Medium', 50, 'Adidas running shoes',      '3rd / 4', 'flat', 6);

  -- trending products
  insert into public.trending_products (user_id, name, category, growth, status, position) values
    (uid, 'Samsung Galaxy S24 FE',   'Electronics', '+45%', 'In catalog', 0),
    (uid, 'Stanley Quencher Tumbler','Home',        '+62%', 'Not listed', 1),
    (uid, 'The Ordinary AHA Peel',   'Beauty',      '+38%', 'Not listed', 2),
    (uid, 'iPad Air M2',             'Electronics', '+29%', 'In catalog', 3),
    (uid, 'Crocs Classic Clog',      'Fashion',     '+51%', 'In catalog', 4);

  -- assortment gaps
  insert into public.assortment_gaps (user_id, product, competitors, price, searches, missed, demand, position) values
    (uid, 'Apple Watch Ultra 2',       '["Carrefour","Amazon.ae","Noon"]'::jsonb,                     3199, '4,200', 'QAR 89K', 'High',   0),
    (uid, 'Samsung Galaxy Tab S9 FE',  '["Talabat","Amazon.ae","Noon"]'::jsonb,                       1499, '3,800', 'QAR 62K', 'High',   1),
    (uid, 'Nespresso Vertuo Pop',      '["Carrefour","Lulu"]'::jsonb,                                  449, '2,100', 'QAR 28K', 'Medium', 2),
    (uid, 'Oura Ring Gen 3',           '["Amazon.ae","Noon"]'::jsonb,                                 1299, '1,400', 'QAR 34K', 'Medium', 3),
    (uid, 'Dyson Airwrap Complete',    '["Carrefour","Amazon.ae","Noon","Lulu"]'::jsonb,              2199, '2,800', 'QAR 51K', 'High',   4),
    (uid, 'PlayStation 5 Slim',        '["Talabat","Carrefour","Lulu","Amazon.ae","Noon"]'::jsonb,    1899, '5,100', 'QAR 94K', 'High',   5);

  -- cross border radar
  insert into public.cross_border_radar (user_id, product, your_price, intl_price, platform, delivery, gap, risk, position) values
    (uid, 'Sony WH-1000XM5',         1299, 1049, 'Amazon Global', '5-8 days',  '-19.2%', 'High',   0),
    (uid, 'Nike Air Max 90',          549,  419, 'Shein',         '7-12 days', '-23.7%', 'High',   1),
    (uid, 'The Ordinary Niacinamide',  45,   29, 'iHerb',         '6-10 days', '-35.6%', 'Medium', 2),
    (uid, 'Apple AirPods Pro 2',      949,  879, 'Amazon Global', '5-8 days',  '-7.4%',  'Medium', 3),
    (uid, 'Dyson V15 Detect',        2799, 2399, 'Noon (UAE)',    '3-5 days',  '-14.3%', 'High',   4);
end;
$$;

-- ========== UPDATE handle_new_user TO CALL SEED ==========
...
begin
  for u in select id from public.profiles loop
    perform public.seed_market_for_user(u.id);
  end loop;
end $$;
