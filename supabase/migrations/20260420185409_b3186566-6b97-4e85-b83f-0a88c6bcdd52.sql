-- =====================================================
-- PROMOTIONS TAB MIGRATION
-- =====================================================

-- 1. promotions_metrics (top KPI cards)
create table public.promotions_metrics (
...
alter table public.promotions_metrics enable row level security;

create policy "Users can view own promotions metrics"
  on public.promotions_metrics for select to authenticated
  using (auth.uid() = user_id);
create policy "Users can insert own promotions metrics"
  on public.promotions_metrics for insert to authenticated
  with check (auth.uid() = user_id);
create policy "Users can update own promotions metrics"
  on public.promotions_metrics for update to authenticated
  using (auth.uid() = user_id);
create policy "Users can delete own promotions metrics"
  on public.promotions_metrics for delete to authenticated
  using (auth.uid() = user_id);

create trigger trg_promotions_metrics_updated
  before update on public.promotions_metrics
  for each row execute function public.set_updated_at();

-- 2. promotion_calendar
create table public.promotion_calendar (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null,
...
alter table public.promotion_calendar enable row level security;

create policy "Users can view own promotion calendar"
  on public.promotion_calendar for select to authenticated
  using (auth.uid() = user_id);
create policy "Users can insert own promotion calendar"
  on public.promotion_calendar for insert to authenticated
  with check (auth.uid() = user_id);
create policy "Users can update own promotion calendar"
  on public.promotion_calendar for update to authenticated
  using (auth.uid() = user_id);
create policy "Users can delete own promotion calendar"
  on public.promotion_calendar for delete to authenticated
  using (auth.uid() = user_id);

create trigger trg_promotion_calendar_updated
  before update on public.promotion_calendar
  for each row execute function public.set_updated_at();

-- 3. past_campaigns
create table public.past_campaigns (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null,
  name text not null,
  discount text not null,
  total_gmv text not null,
  incremental_gmv text not null,
  cannibalized text not null,
...
alter table public.past_campaigns enable row level security;

create policy "Users can view own past campaigns"
  on public.past_campaigns for select to authenticated
  using (auth.uid() = user_id);
create policy "Users can insert own past campaigns"
  on public.past_campaigns for insert to authenticated
  with check (auth.uid() = user_id);
create policy "Users can update own past campaigns"
  on public.past_campaigns for update to authenticated
  using (auth.uid() = user_id);
create policy "Users can delete own past campaigns"
  on public.past_campaigns for delete to authenticated
  using (auth.uid() = user_id);

create trigger trg_past_campaigns_updated
  before update on public.past_campaigns
  for each row execute function public.set_updated_at();

-- 4. timing_insights
create table public.timing_insights (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null,
  title text not null,
  body text not null,
...
create policy "Users can view own timing insights"
  on public.timing_insights for select to authenticated
  using (auth.uid() = user_id);
create policy "Users can insert own timing insights"
  on public.timing_insights for insert to authenticated
  with check (auth.uid() = user_id);
create policy "Users can update own timing insights"
  on public.timing_insights for update to authenticated
  using (auth.uid() = user_id);
create policy "Users can delete own timing insights"
  on public.timing_insights for delete to authenticated
  using (auth.uid() = user_id);

create trigger trg_timing_insights_updated
  before update on public.timing_insights
  for each row execute function public.set_updated_at();
...
create or replace function public.seed_promotions_for_user(uid uuid)
returns void
language plpgsql
security definer
...
begin
  -- promotions_metrics
  insert into public.promotions_metrics (user_id, slug, label, value, value_color, footer_text, footer_color, position) values
    (uid, 'active_competitor_promos', 'Active competitor promos', '8', '#1A1A18', 'running right now', '#6B6B6B', 0),
    (uid, 'upcoming_7_days', 'Upcoming in 7 days', '3', '#1A1A18', 'across 3 competitors', '#6B6B6B', 1),
    (uid, 'last_campaign_roi', 'Your last campaign ROI', '1.4x', '#F59E0B', 'Eid Electronics Blitz', '#6B6B6B', 2),
    (uid, 'avg_cannibalization', 'Avg cannibalization rate', '38%', '#EF4444', 'across last 3 campaigns', '#6B6B6B', 3);

  -- promotion_calendar
  insert into public.promotion_calendar (user_id, competitor, campaign, channel, dates, duration, depth, categories, status, position) values
    (uid, 'Talabat',   'Eid Al-Fitr Mega Sale',          'Both',     'Mar 28 - Apr 5',   '9 days',    '15-25%',        'All categories',          'Live',     0),
    (uid, 'Carrefour', 'Back to School',                 'Both',     'Apr 10 - Apr 20',  '11 days',   '10-20%',        'Electronics, Stationery', 'Upcoming', 1),
    (uid, 'Lulu',      'Weekend Flash Deals',            'In-Store', 'Every Thu-Sat',    'Recurring', '5-15%',         'Grocery, Home',           'Live',     2),
    (uid, 'Amazon.ae', 'Spring Deals',                   'Online',   'Apr 15 - Apr 22',  '8 days',    '20-40%',        'Electronics, Fashion',    'Upcoming', 3),
    (uid, 'Noon',      'Yellow Friday Mini',             'Online',   'Apr 18 - Apr 19',  '2 days',    '10-30%',        'Electronics',             'Upcoming', 4),
    (uid, 'Carrefour', 'Doha Festival City Exclusive',   'In-Store', 'Apr 1 - Apr 14',   '14 days',   '10-15%',        'Home, Beauty',            'Live',     5),
    (uid, 'Talabat',   'Free Delivery Week',             'Online',   'Apr 7 - Apr 13',   '7 days',    'Free delivery', 'All categories',          'Live',     6),
    (uid, 'Lulu',      'Ramadan Clearance',              'Both',     'Mar 20 - Mar 30',  '11 days',   '20-40%',        'Grocery, Home',           'Ended',    7);

  -- past_campaigns
  insert into public.past_campaigns (user_id, name, discount, total_gmv, incremental_gmv, cannibalized, roi, verdict, position) values
    (uid,
     'Eid Electronics Blitz (Mar 2026)',
     ...
     0),
    (uid,
     'Free Delivery Weekend (Feb 2026)',
     ...
     1),
    (uid,
     'Flash Sale: Headphones (Jan 2026)',
     '30% off premium headphones',
     '+QAR 47K',
     '+QAR 12K',
     'QAR 35K (74%)',
     0.6,
  ...
  -- timing_insights
  insert into public.timing_insights (user_id, title, body, position) values
    (uid,
     'Optimal promotion windows for Snoonu',
     'Based on 11 months of competitive tracking, the best windows for your promotions are: Monday through Wednesday for grocery (avoiding Lulu''s Thursday to Saturday flash deals), Friday evening for electronics (after Carrefour''s Thursday price drop traffic has peaked), and the first week after any major Talabat sale ends (to capture deal-seekers still browsing).',
     0);
end;
$$;

-- =====================================================
-- UPDATE handle_new_user TO INCLUDE PROMOTIONS SEEDING
-- =====================================================
...
begin
  for p in select id from public.profiles loop
    if not exists (select 1 from public.promotions_metrics where user_id = p.id) then
      perform public.seed_promotions_for_user(p.id);
    end if;
  end loop;
end;
$$;
